...
export default function Reports() {
  ...
  if (sortConfig.key) {
    filtered = [...filtered].sort((a, b) => {
      ...
      // Numeric sorting for Amount and Qty
      ...
      <div className="min-h-screen bg-gradient-to-br from-[#0a1628] to-[#1a2332] text-white p-2 sm:p-4">
        <div className="max-w-full mx-auto bg-gradient-to-br from-[#12243d] to-[#0f1e33] rounded-2xl p-3 sm:p-5 border border-[#1e3553] shadow-2xl">
          ...
          <div className="flex flex-wrap gap-2 bg-gradient-to-r from-[#0f1e33] to-[#1a2a45] p-2 mb-3 rounded-xl border border-[#1e3553]">
            ...
            <button onClick={handleExportExcel} className="bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 px-4 py-2 rounded-lg font-bold text-xs transition-all shadow-lg">📊 Excel</button>
            <button onClick={handleExportPDF} className="bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 px-4 py-2 rounded-lg font-bold text-xs transition-all shadow-lg">📄 PDF</button>
          </div>
            ...
            >
              <table className="border-collapse text-[10px] sm:text-xs w-full">
                ...
                <tbody>
                  {pageRows.length === 0 ? (
                    <tr>
                      <td colSpan={EXCEL_COLUMNS.length} className="text-center py-12 text-gray-400 text-sm">
                    ...
                    <>
                      {pageRows.map((row, i) => (
                        <tr key={i} className={`${i % 2 ? "bg-[#0f1e33]" : "bg-[#132a4a]"} hover:bg-[#1a3d5e] transition-colors`}>
                          ...
                          <td className="px-3 py-2 border-r border-[#1e3553] whitespace-nowrap">{row["Voucher Number"] || "—"}</td>
                          <td className="px-3 py-2 border-r border-[#1e3553] whitespace-nowrap">{row["Voucher Type"] || "—"}</td>
                          <td className="px-3 py-2 border-r border-[#1e3553] font-medium whitespace-nowrap" style={{maxWidth: "200px"}} title={row["Party Name"]}>{row["Party Name"] || "—"}</td>
                          <td className="px-3 py-2 border-r border-[#1e3553] text-blue-300 whitespace-nowrap" style={{maxWidth: "200px"}} title={row["Item Name"]}>{row["Item Name"] || "—"}</td>
                          ...
                          <td className="px-3 py-2 text-xs text-gray-400 whitespace-nowrap" style={{maxWidth: "200px"}} title={row["Narration"]}>{row["Narration"] || "—"}</td>
                        </tr>
                      ))}
